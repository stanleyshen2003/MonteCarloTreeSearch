...
public:
    ...
    MCTSNode(const GameState& state, MCTSNode* parent = nullptr) : state(state), parent(parent), visits(0), score(0) {}

    MCTSNode* select_child();
    MCTSNode* add_child(const GameState&);
    
    double calculate_ucb(MCTSNode*);
};

// Add a child node to this node
MCTSNode* MCTSNode::add_child(const GameState& state) {
...
double MCTSNode::calculate_ucb(MCTSNode* node) {
    ...
    return (node->score / node->visits) + sqrt(2 * log(visits) / node->visits);
}
...
MCTSNode* MCTS_agent::expand_node(MCTSNode* node) {
    GameState state_copy = node->state; // Make a copy of the state to modify
    vector<Action> actions = state_copy.get_actions(); // Get all possible actions
    if (actions.empty()) {
        return node; // No valid actions to expand, return the same node
    }
    Action random_action = actions[rand() % actions.size()]; // Choose a random action
    state_copy.next_state(random_action); // Update the state with the chosen action
    MCTSNode* child_node = node->add_child(state_copy); // Add a child node with the updated state
    return child_node;
}
...
// Delete the entire Monte Carlo Tree recursively
...
Action MCTS_agent::decide_step(GameState& state) {
    ...
    //             Action non_action(-1, -1, -1, -1);
    //             new_state.next_state(non_action);
    //         }
    //         else{
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        // Selection phase: Choose a node using Upper Confidence Bound (UCB) criteria
        MCTSNode* selected_node = select_node(root);

        // Expansion phase: Expand the selected node by adding a child node
        MCTSNode* expanded_node = expand_node(selected_node);

        // Simulation (Rollout) phase: Simulate a game from the expanded node to get a score
        int simulation_result = rollout(expanded_node->state);

        // Backpropagation phase: Update statistics of all nodes from the expanded node to the root
